...
module Sentinel
  ...
  class Server < Sinatra::Base
    ...
    post '/travis' do
      ...
      unless verify_travis(build, signature)
      ...
      build['branch'] =~ %r{/^sentinel\/#{build["repository"]["owner_name"]}\/#{build["repository"]["name"]}\/(\d+)/}
      if Regexp.last_match(1)
        pr = Regexp.last_match(1)
        puts "I have #{pr}"
        if build['result'].zero?
          Sentinel.github.issues.comments.create(
    ...
    post '/payload' do
      ...
      if pr.key?('pull_request') && pr['action'] == 'opened'
        Hub.opened_pr(pr)
      elsif pr.key?('comment') && pr.key?('issue') && (pr['action'] == 'created' || pr['action'] == 'edited')
...
unless File.exist?('/hab/svc/sentinel/config.toml')
...
config = CONFIG
unless config['cfg']['login']
  puts 'You must specify cfg.login in config.toml'
  exit 1
end

unless config['cfg']['access_token']
